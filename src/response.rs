...
//         self.write(status_code, text);
//     }

//     pub fn file(&mut self, content: &str, opts: Option<ResponseOptions>) {
//         let status_code = opts.map(|opts| opts.status_code).unwrap_or(StatusCode::Ok);
...
//         self.write(status_code, Some(content));
//     }
...
//     fn write(&mut self, status_code: StatusCode, body: Option<&str>) {
//         if self.compression_schemas.is_empty() {
//             self.write_decompress_response(status_code, body)
//         } else {
//             self.write_compress_response(status_code, body)
//         };
//     }

//     fn write_compress_response(&mut self, status_code: StatusCode, body: Option<&str>) {
...
//         let body: Option<Vec<u8>> = body
//             .map(|v| {
//                 let schema = self.compression_schemas.first().unwrap();

//                 match schema.compress(v) {
//                     Ok(compressed_body) => {
//                         self.headers.insert(
...
//         let response = format!("{} {}{}\r\n\r\n", self.version, status_code, headers_string);

//         self.stream
//             .write_all(response.as_bytes())
//             .expect("Could not write the response");
...
//     fn write_decompress_response(&mut self, status_code: StatusCode, body: Option<&str>) {
//         let body: Option<String> = body
//             .map(|v| {
//                 self.headers
//                     .insert("Content-Length".to_string(), v.len().to_string());
